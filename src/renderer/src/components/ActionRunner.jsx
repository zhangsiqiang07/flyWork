    ...
    try {
      ...
      if (window.flywork?.executeAction) {
        ...
        result = {
          success: true,
          output: dry
            ? `[DRY RUN] ${MOCK_OUTPUTS[action.id] || '命令将被执行'}`
            : MOCK_OUTPUTS[action.id] || '执行成功'
        }
      }
      const finalOutput =
        result.output ||
        result.error ||
        (result.success ? `✓ ${action.name} 执行成功` : `❌ ${action.name} 执行失败`)
        ...
        <div className="modal-body">
          ...
          {(phase === 'running' || phase === 'done') && (
            <div>
              ...
              >
                {phase === 'running' && (
                  <span className="animate-spin" style={{ display: 'inline-block', fontSize: 12 }}>
                    ⟳
                  </span>
                )}
