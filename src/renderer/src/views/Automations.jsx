...
export default function AutomationsView({
  ...
  const selectedAutomation =
    automations.find((a) => a.id === selectedAutomationId) || automations[0] || null
  ...
  const handleRunStep = async (autoId, step, dryRun = false) => {
    ...
    try {
      if (window.flywork?.executeAutomationStep) {
        result = await window.flywork.executeAutomationStep(
          resolvedCmd,
          ws?.root,
          mergedEnv,
          dryRun,
        ...
        result = {
          ...
          exitCode: 0,
          output: dryRun
            ? `[Dry Run 模式] 将" ${ws?.root || '当前目录'} 执行: ${resolvedCmd}\n环境变量: ${JSON.stringify(mergedEnv)}`
            : `[执行完成] 命令: ${resolvedCmd}\n输出: 成功完成处理`,
          error: ''
        }
      }
    ...
    setAutomations((prev) =>
      prev.map((a) => {
        ...
        const nextSteps = a.steps.map((s) =>
          s.id === step.id ? { ...s, status: result.success ? 'complete' : 'failed' } : s
        )
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`badge badge-${auto.lastStatus === 'success' ? 'green' : auto.lastStatus === 'failed' ? 'red' : 'gray'}`}
              ...
              <div>
                ...
                {selectedWorkspace && (
                  <div style={{ fontSize: 11, color: 'var(--accent-blue)', marginTop: 4 }}>
                    📍 绑定工"区: {selectedWorkspace.icon} {selectedWorkspace.name} (
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                {isBatchRunning ? (
                  ...
                  <>
                    <button
                      className="btn btn-ghost btn-sm"
                      onClick={() => handleRunAll(selectedAutomation, true)}
                    >
                      ⚡ 全部 Dry Run
                    </button>
                    <button
                      className="btn btn-primary btn-sm"
                      onClick={() => handleRunAll(selectedAutomation, false)}
                    >
                      ▶ 运行全部
                    </button>
                  </>
              ...
              {selectedAutomation.steps.map((step, idx) => {
                ...
                return (
                  <div
                    key={step.id || idx}
                    className="flow-step"
                    style={{ paddingBottom: idx < selectedAutomation.steps.length - 1 ? 20 : 0 }}
                  >
                    {idx < selectedAutomation.steps.length - 1 && (
                      <div className="flow-step-line" />
                    )}
                    <div
                      className={`flow-step-icon ${runStatus === 'complete' ? 'complete' : runStatus === 'running' || runStatus === 'dryrun' ? 'running' : runStatus === 'failed' ? 'failed' : ''}`}
                      style={{ position: 'relative', zIndex: 1 }}
                    >
                      {runStatus === 'complete' && (
                        <span style={{ color: 'var(--accent-green)', fontSize: 14 }}>"</span>
                      )}
                      {(runStatus === 'running' || runStatus === 'dryrun') && (
                        <span
                          style={{
                            fontSize: 12,
                    ...
                    <div className="flow-step-content" style={{ width: '100%' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          {step.env && Object.keys(step.env).length > 0 && (
                            <span
                              ...
                              style={{ fontSize: 10 }}
                              title={`步骤专属变量: ${JSON.stringify(step.env)}`}
                            >
                              步骤专属 ENV
                            </span>
                          )}
                        ...
                        {log?.exitCode !== undefined && (
                          <span
                            style={{
                              fontSize: 10,
                              color:
                                log.exitCode === 0 ? 'var(--accent-green)' : 'var(--accent-red)',
                              fontFamily: 'monospace'
                            }}
                          >
                        ...
                        (() => {
                          ...
                          return (
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', gap: 6 }}>
                                  {fullText && (
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        handleCopyText(fullText, `full-${stepKey}`)
                                      }}
                                    >
                              ...
                              <div
                                ref={(el) => {
                                  logScrollRefs.current[stepKey] = el
                                }}
                                style={{
                                  padding: '10px 14px',
                                  fontSize: 11,
                      ...
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        <button
                          ...
                          disabled={
                            runStatus === 'running' || runStatus === 'dryrun' || isBatchRunning
                          }
                          style={{ fontSize: 11 }}
                        >
                          Dry Run
                        ...
                        {runStatus === 'complete' && (
                          <span
                            style={{ fontSize: 11, color: 'var(--accent-green)', marginLeft: 4 }}
                          >
                            " 执行成功
                          </span>
                        )}
