...
export default function WorkspaceDetail({
  ...
  const runAutomation = async (automation, dryRun = false) => {
    ...
    if (
      !dryRun &&
      highRiskSteps.length &&
      !window.confirm(
        `⚠️ 此流程包含 ${highRiskSteps.length} 个高风险步骤，确认在当前工作目录执行吗？\n\n${ws.root}`
      )
    )
      return
    ...
    try {
      ...
      const success =
        results.length === automation.steps.length && results.every((result) => result.success)
      setAutomationLogs((prev) => ({ ...prev, [automation.id]: { dryRun, success, results } }))
      onUpdateAutomations?.((prev) =>
        prev.map((item) =>
  ...
  const refreshGitData = useCallback(async () => {
    ...
    try {
      const [info, branchesRes, logs] = await Promise.all([
        ...
        window.flywork?.gitGetBranches
          ? window.flywork.gitGetBranches(ws.root)
  ...
  const loadNativeSessions = useCallback(async () => {
    ...
    const targetPath = selectedAgentTab.includes('Claude')
      ? ws.claudeProjectPath || ws.root
      : ws.codexProjectPath || ws.root
  ...
  const currentBuildStatus =
    latestAutomation?.lastStatus === 'success'
      ? 'success'
      : latestAutomation?.lastStatus === 'failed'
        ? 'failed'
        : ws.buildStatus
  const currentBuildName = latestAutomation
    ? `自动化 · ${latestAutomation.name}`
    : ws.integrations?.jenkins || 'CI Build'
  const currentBuildMessage = latestAutomation
    ? latestAutomation.lastStatus === 'success'
      ? '自动化流程执行成功'
      : '自动化流程执行失败'
    : ws.buildMessage
  const currentBuildTime = latestAutomation?.lastRun
    ? formatRelTime(latestAutomation.lastRun)
    : ws.buildTime
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {activeSession && (
                <span
                  ...
                  style={{ fontSize: 10, animation: 'pulseGlow 2s infinite' }}
                >
                  🟢 工作中
                </span>
              )}
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {activeSession ? (
              <button
                ...
                onClick={() => onPauseSession(activeSession.id)}
              >
                ⏸ 暂停会话
              </button>
            ) : (
        ...
        {showNewSession && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 8, animation: 'fadeIn 150ms ease' }}>
            <input
              ...
              autoFocus
            />
            <button className="btn btn-primary btn-sm" disabled={!newSessionTitle.trim()}>
              创建会话
            </button>
            <button
              className="btn btn-ghost btn-sm"
            ...
            <div className="card" style={{ padding: 16 }}>
              ...
              {currentBuildStatus === 'failed' && (
                <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                  <button className="btn btn-ghost btn-sm" onClick={() => setActiveTab('构建')}>
                    查看构建
                  </button>
                  <button
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{ background: risk.bg, fontSize: 14 }}
                      >
                        {action.icon}
                      </div>
                      <span className="action-item-name">{action.name}</span>
                      <span
                        className="badge action-item-risk"
                        style={{ background: risk.bg, color: risk.color, fontSize: 10 }}
                      >
                        {risk.label}
                      </span>
                    </div>
              ...
              return (
                <div key={risk} style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <span
                      className="badge"
                      style={{ background: riskConf.bg, color: riskConf.color, fontSize: 10 }}
                    >
                      {riskConf.label}
                    </span>
        ...
        {activeTab === '构建' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="card" style={{ padding: 16 }}>
              <div
                style={{
                  ...
                  justifyContent: 'space-between',
                  gap: 12,
                  marginBottom: automations.length ? 12 : 0
                }}
              >
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 3 }}>
                    已绑定此工作空间的流程会直接在{' '}
                    <span style={{ fontFamily: 'monospace' }}>{ws.root}</span> 中执行。
                  </div>
                </div>
                <span className="badge badge-blue" style={{ fontSize: 10, flexShrink: 0 }}>
                  {automations.length} 个流程
                </span>
              </div>
                  ...
                  {automations.map((automation) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3 }}>
                              {automation.steps?.length || 0} 步骤 ·{' '}
                              {automation.description || '未填写描述'}
                            </div>
                          </div>
                          {runLog && !isRunning && (
                            <span
                              className={`badge badge-${runLog.success ? 'green' : 'red'}`}
                              style={{ fontSize: 10 }}
                            >
                              {runLog.dryRun
                                ? '预演完成'
                          ...
                          <button
                            className="btn btn-ghost btn-sm"
                            style={{ fontSize: 11 }}
                            disabled={Boolean(runningAutomationId)}
                            onClick={() => runAutomation(automation, true)}
                          >
                            Dry Run
                          </button>
                          <button
                          ...
                          <button
                            ...
                            title="显示步骤与日志"
                            onClick={() =>
                              setExpandedAutomationId(isExpanded ? null : automation.id)
                            }
                          >
                            ⌄
                          </button>
                        ...
                        {isExpanded && (
                          <div
                            style={{ padding: '0 12px 12px', borderTop: '1px solid var(--border)' }}
                          >
                            <ol
                              style={{
                                margin: '10px 0',
                            ...
                            >
                              {automation.steps.map((step) => (
                                <li key={step.id}>
                                  ...
                                  >
                                    {' '}
                                    · {resolveAutomationCommand(step.command)}
                                  </span>
                                </li>
                              ))}
                            </ol>
                            {runLog && (
                              <div
                                ...
                                style={{ maxHeight: 220, fontSize: 11 }}
                              >
                                {runLog.results
                                  .map(
                                    (result) =>
            ...
            <div
              ...
              style={{
                borderColor:
                  ws.buildStatus === 'failed' ? 'rgba(224,92,92,0.3)' : 'rgba(63,185,80,0.2)'
              }}
            >
              <div className="build-status-card">
                <div
                  className={`build-status-icon ${ws.buildStatus}`}
                  style={{ width: 44, height: 44, fontSize: 20 }}
                >
                  {ws.buildStatus === 'success' ? '✅' : '❌'}
                </div>
                <div className="build-status-info">
                  <div className="build-status-name" style={{ fontSize: 15 }}>
                    {ws.integrations?.jenkins || 'CI Build'}
                  </div>
                  <div
                    className="build-status-desc"
                    style={{
                      color:
                        ws.buildStatus === 'failed' ? 'var(--accent-red)' : 'var(--accent-green)',
                      fontSize: 13,
                      marginTop: 3
                    }}
                  >
                  ...
                  {ws.buildDuration && (
                    <div className="build-status-meta" style={{ marginTop: 4 }}>
                      耗时 {ws.buildDuration} · {ws.buildTime}
                    </div>
                  )}
                </div>
                {onNavigate && (
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            ...
                            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                              {session.status === 'active'
                                ? '进行中'
                                : `暂停于 ${formatRelTime(session.updatedAt)}`}
                            </span>
                          </div>
                          {session.notes && (
                            <div
                              ...
                              onClick={() => {
                                ;(onOpenSessionChat || onSetContextPanel)?.({
                                  ...
                                  messages: [
                                    {
                                      role: 'user',
                                      content: `任务名称：${session.title}\n分支：${session.branch}\n状态：${session.status === 'active' ? '进行中' : '已暂停'}`,
                                      time: formatRelTime(session.updatedAt)
                                    },
                                    session.notes
                                      ? {
                                          role: 'assistant',
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    <span>
                      只过滤显示当前工程{' '}
                      <strong style={{ color: 'var(--accent-blue)' }}>{ws.name}</strong>{' '}
                      对应的智能体会话列表
                    </span>
                    <span className="badge badge-gray" style={{ fontSize: 9, opacity: 0.8 }}>
                      {selectedAgentTab.includes('ChatGPT')
                        ? '📌 对齐应用侧边栏与最新时间排序'
                        : '🕒 按最近交互时间倒序'}
                    </span>
                  </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {nativeAgentSessions.length > 0 ? (
                        ...
                        >
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              <span>
                                Session:{' '}
                                <code style={{ fontSize: 10 }}>{nSess.sessionId.slice(0, 12)}</code>
                              </span>
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation()(onOpenSessionChat || onSetContextPanel)?.({
                                type: 'native-session',
                                sessionId: nSess.sessionId,
                                summary: nSess.summary,
                                agent: selectedAgentTab,
                    ...
                    {nativeAgentSessions.length > currentPage * pageSize ? (
                      ...
                      >
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          已显示{' '}
                          <strong style={{ color: 'var(--text-primary)' }}>
                            {Math.min(currentPage * pageSize, nativeAgentSessions.length)}
                          </strong>{' '}
                          / 共 <strong>{nativeAgentSessions.length}</strong> 条历史会话
                        </div>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button
                            className="btn btn-secondary btn-sm"
                          ...
                          <button
                            ...
                            style={{ fontSize: 11, color: 'var(--accent-blue)' }}
                            onClick={() =>
                              setCurrentPage(Math.ceil(nativeAgentSessions.length / pageSize))
                            }
                          >
                  ...
                  >
                    未在 <strong>{selectedAgentTab}</strong> 中找到工程{' '}
                    <strong style={{ color: 'var(--accent-blue)' }}>{ws.name}</strong>{' '}
                    的专属历史会话。
          ...
          <div className="timeline">
            {activityLog.length === 0 ? (
              <div className="empty-state">
                <div className="empty-state-title">暂无活动记录</div>
              </div>
            ) : (
